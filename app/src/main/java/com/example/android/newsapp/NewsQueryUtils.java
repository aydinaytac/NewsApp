        ...
        try {
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                for (int j = 0; j < tags.length(); j++) {
                    JSONObject contributorObject = tags.getJSONObject(j);
                    contributor = contributorObject.getString("webTitle");
                }

                // Create a new {@link News} object with the title, contributor
                // and url from the JSON response.
                News news = new News(title, contributor, date, section, webUrl);
